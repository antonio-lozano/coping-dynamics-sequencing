...
IC = pd.read_csv(HERE / "internal_consistency.csv")
TC = pd.read_csv(HERE / "timecourse_reproduction.csv")
OUT = HERE / "2026_06_14_statistics_validation.html"
...
def stat_card(label, value, sub, kind="default"):
    ...
    return (f'<div class="{cls}"><div class="label">{esc(label)}</div>'
...
def svg_z_scatter(df, width=520, height=360):
    ...
    def X(v): return 60 + (v - lo) / (hi - lo) * (width - 90)
    def Y(v): return height - 45 - (v - lo) / (hi - lo) * (height - 70)
    p = [f'<svg viewBox="0 0 {width} {height}" xmlns="http://www.w3.org/2000/svg">']
    p.append(f'<rect class="axis" x="60" y="25" width="{width-90}" height="{height-70}" fill="none" stroke-width="1"/>')
    # identity line y=x
    p.append(f'<line x1="{X(lo):.1f}" y1="{Y(lo):.1f}" x2="{X(hi):.1f}" y2="{Y(hi):.1f}" stroke="#6ec48a" stroke-width="1.5" stroke-dasharray="4,4"/>')
    # ticks
    step = 2
    t = 0
    while t <= hi:
        p.append(f'<line class="grid" x1="{X(t):.1f}" y1="25" x2="{X(t):.1f}" y2="{height-45}"/>')
        p.append(f'<text x="{X(t):.1f}" y="{height-28}" font-size="10" text-anchor="middle">{t}</text>')
    ...
    for xr, yp_, lab in zip(zr, zp, labels):
        ...
        p.append(f'<text x="{X(xr)+8:.1f}" y="{Y(yp_)-6:.1f}" font-size="10" fill="#8b94a3">{esc(lab)}</text>')
    p.append(f'<text x="{width/2:.0f}" y="{height-8}" font-size="11" text-anchor="middle" fill="#8b94a3">|z| reported (manuscript)</text>')
    p.append(f'<text x="16" y="{height/2:.0f}" font-size="11" text-anchor="middle" fill="#8b94a3" transform="rotate(-90 16 {height/2:.0f})">|z| reproduced</text>')
    p.append('</svg>')
...
def ic_rows():
    ...
    for _, r in IC.iterrows():
        ...
        badge = f'<span style="color:#e88871;font-weight:600">{esc(fl)}</span>' if fl else '<span style="color:#6ec48a">ok</span>'
        out.append(
...
ERR_EXPL = {
    "Fig3A sniffing frequency": ("z != beta/SE",
    ...
    "Fig5C grooming (resil contrast)": ("p disagrees with z",
    ...
    "Fig5C turn (resil contrast)": ("non-significant reported as p<0.001",
...
.venv/Scripts/python.exe scripts/analysis/manuscript_statistics.py     # prints tables, writes CSVs
.venv/Scripts/python.exe results/manuscript_statistics/build_validation_report.py   # rebuilds this report</pre>
<details><summary>Files</summary>
<pre>scripts/analysis/manuscript_statistics.py            # the validation (Layer 1 + Layer 2)
results/manuscript_statistics/internal_consistency.csv
results/manuscript_statistics/timecourse_reproduction.csv
results/manuscript_statistics/2026_06_14_statistics_validation.html  # this file</pre>
</details>
</section>
</main></div>
