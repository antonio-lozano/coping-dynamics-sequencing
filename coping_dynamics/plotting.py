    ...
    for i in range(n):
        for j in range(n):
            ...
            if weight_ij > 0:
                ...
                alpha = (
                    edge_alpha
                    if edge_alpha_min is None
                    else edge_alpha_min + rel_weight * (edge_alpha - edge_alpha_min)
                )
            else:
            ...
            patch = PathPatch(path, facecolor="none", edgecolor=edge_color, lw=lw, alpha=alpha)
            ax.add_patch(patch)
...
def plot_difference_network(diff_matrix, labels, colors, title=None, ax=None, figsize=(10, 8)):
    ...
    node_sizes = [
        base_size + additional_size_factor * (abs(node_effect[node]) / node_abs_max)
        for node in labels
    ]
    ...
    for (source, target), color, width in zip(edges, edge_colors, edge_widths):
        ...
        path = Path([source_pos, control_pos, target_pos], [Path.MOVETO, Path.CURVE3, Path.CURVE3])
    ...
    nx.draw_networkx_labels(
        G, label_pos, font_size=9, font_weight="normal", font_color="#4D4D4D", ax=ax
    )
...
def add_difference_colorbar(
    fig, cmap, norm, position=[0.85, 0.25, 0.02, 0.5], label="Δ Probability"
):
    """
