...
BODYPARTS = ["nose", "H1R", "H2R", "H1L", "H2L", "B1R", "B2R", "B3R",
             "B1L", "B2L", "B3L", "tail", "S2", "S1"]
SKELETON_EDGES = [(0, 1), (1, 2), (0, 3), (3, 4), (0, 5), (0, 8),
                  (5, 6), (6, 7), (8, 9), (9, 10), (7, 11), (10, 11),
                  (11, 12), (12, 13)]
...
def snap_quad_to_edges(grey, quad, search=34, samples=90, smooth=2.0):
    ...
    for i in range(4):
        ...
        proj = found @ normal
        keep = np.abs(proj - np.median(proj)) <= 2.5 * (np.median(np.abs(proj - np.median(proj))) + 1e-6)
...
def detect_floor(background: np.ndarray) -> np.ndarray:
    ...
    print(f"  floor: threshold {best[0]}, rectangularity {best[2]:.3f}, "
...
def draw_frame(frame: np.ndarray, inside: np.ndarray, floor: np.ndarray,
               points: np.ndarray, likelihood: np.ndarray,
               index: int, climbing: bool) -> Image.Image:
    ...
    label = font(26, True)
    small = font(20)
    draw.text((16, 12), f"Floor {ratio:.2f}  |  Wall {1 - ratio:.2f}", font=label,
              fill=(255, 255, 0), stroke_width=2, stroke_fill=(40, 40, 40))
    draw.text((16, 46), f"Frame {index}", font=small, fill=(255, 255, 255),
              stroke_width=2, stroke_fill=(40, 40, 40))
    if climbing:
        draw.text((16, 74), "CLIMBING", font=label, fill=(246, 166, 90),
                  stroke_width=2, stroke_fill=(40, 40, 40))
    return image
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--bouts", type=int, default=6,
                        help="number of climbing bouts to render")
    parser.add_argument("--context", type=int, default=10,
                        help="frames of lead-in/lead-out around each bout")
...
def main() -> None:
    ...
    print(f"  {len(bouts)} climbing bouts, {climbing_frames} frames "
    ...
    writer = imageio_ffmpeg.write_frames(
        str(args.output), (width, height), fps=FPS, codec="libx264",
    ...
    for i in wanted:
        if i not in frames:
            continue
        image = draw_frame(frames[i], inside, floor, keypoints[i, :, :2],
                           keypoints[i, :, 2], i, i in climbing_set)
    ...
    sidecar.write_text(json.dumps({
        ...
        "climbing_fraction_of_tracked": round(float(climbing_frames / max(valid.sum(), 1)), 4),
