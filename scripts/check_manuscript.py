...
SHIPPED_MS = REPO / "report" / "Sanguino-Gómez_coping_strategies_Nature_Neuroscience_highlighted.docx"
...
def C(panel, ms, *, scale=1, csv_scale=1, csv=None, wb=None, note=""):
    ...
    """
    return dict(panel=panel, ms=ms, scale=scale, csv_scale=csv_scale,
                csv=csv, wb=wb, note=note)
...
CLAIMS = [
    C("Fig 2A time", (3.721, 0.128, 29.048), scale=2,
      csv=("stats_figure2_ground_truth_MixedLM.csv",
           {"section": "Sanguino-Gomez & Krugers", "parameter": "Time (per minute)"},
           "coef", "se", "z", "p_value"), csv_scale=2,
      wb=("Fig.2A-C", "Sanguino-Gomez & Kru", "Time (per minute)", 0),
      note="per 30 s bin"),
    C("Fig 2A ELS x time", (-0.887, 0.181, -4.898), scale=2,
      csv=("stats_figure2_ground_truth_MixedLM.csv",
           {"section": "Sanguino-Gomez & Krugers", "parameter": "Stress x time (per minute)"},
           "coef", "se", "z", "p_value"), csv_scale=2,
      wb=("Fig.2A-C", "Sanguino-Gomez & Kru", "Stress x time", 0), note="per 30 s bin"),
    C("Fig 2B time", (4.412, 0.191, 23.091), scale=2,
      csv=("stats_figure2_ground_truth_MixedLM.csv",
           {"section": "Sanguino-Gomez et al.", "parameter": "Time (per minute)"},
           "coef", "se", "z", "p_value"), csv_scale=2,
      wb=("Fig.2A-C", "Sanguino-Gomez et al", "Time (per minute)", 0), note="per 30 s bin"),
    C("Fig 2B ELS x time", (-0.721, 0.270, -2.667), scale=2,
      csv=("stats_figure2_ground_truth_MixedLM.csv",
           {"section": "Sanguino-Gomez et al.", "parameter": "Stress x time (per minute)"},
           "coef", "se", "z", "p_value"), csv_scale=2,
      wb=("Fig.2A-C", "Sanguino-Gomez et al", "Stress x time", 0), note="per 30 s bin"),
    C("Fig 2C ELS x time", (-0.822, 0.154, -5.327), scale=2,
      csv=("stats_figure2_ground_truth_MixedLM.csv",
           {"section": "Combined full dataset", "parameter": "Stress x time (per minute)"},
           "coef", "se", "z", "p_value"), csv_scale=2,
      wb=("Fig.2A-C", "Combined full", "Stress x time", 0), note="per 30 s bin"),
    C("Fig 2H time", (0.126, 0.004, 32.620), scale=60,
      csv=("stats_figure2h_syllables_MixedLM.csv",
           {"section": "Syllables 0 and 28 - Combined full dataset", "parameter": "Time (per minute)"},
           "coef", "se", "z", "p_value"), csv_scale=60,
      wb=("Fig.2H", "Syllables 0 and 28", "Time (per minute)", 0), note="per second"),
    C("Fig 2H ELS x time", (-0.023, 0.005, -4.262), scale=60,
      csv=("stats_figure2h_syllables_MixedLM.csv",
           {"section": "Syllables 0 and 28 - Combined full dataset", "parameter": "Stress x time (per minute)"},
           "coef", "se", "z", "p_value"), csv_scale=60,
      wb=("Fig.2H", "Syllables 0 and 28", "Stress x time", 0), note="per second"),

    C("Fig 3A freeze", (-0.204, 0.081, -2.510),
      csv=("stats_figure3A_GEE.csv",
           {"cluster": "Freeze", "parameter": "C(group, Treatment('Control'))[T.ELS]"},
           "beta", "se", "z", "p_value"),
      wb=("Fig.3A", "Freeze", "Stress", 0)),
    C("Fig 3A sniff", (0.623, 0.231, 2.696),
      csv=("stats_figure3A_GEE.csv",
           {"cluster": "Sniff", "parameter": "C(group, Treatment('Control'))[T.ELS]"},
           "beta", "se", "z", "p_value"),
      wb=("Fig.3A", "Sniff", "Stress", 0)),
    C("Fig 3A turn", (0.101, 0.032, 3.099),
      csv=("stats_figure3A_GEE.csv",
    ...
    C("Fig 3B freeze x time", (-0.023, 0.005, -4.262), scale=60,
      csv=("stats_figure3_overtime.csv",
           {"cluster": "Freeze", "analysis": "Combined",
            "parameter": "group[T.ELS]:time_bin_numeric"},
           "coef", "std_err", "z", "p_value"), csv_scale=1,
      wb=("Fig.3B-H", "Freeze", "Stress x time", 0), note="per second"),
    C("Fig 3C sniff x time", (-0.014, 0.004, -3.656), scale=60,
      csv=("stats_figure3_overtime.csv",
           {"cluster": "Sniff", "analysis": "Combined",
            "parameter": "group[T.ELS]:time_bin_numeric"},
           "coef", "std_err", "z", "p_value"), csv_scale=1,
      wb=("Fig.3B-H", "Sniff", "Stress x time", 0), note="per second"),
    C("Fig 3E turn x time", (0.026, 0.007, 3.610), scale=60,
      csv=("stats_figure3_overtime.csv",
           {"cluster": "Turn", "analysis": "Combined",
            "parameter": "group[T.ELS]:time_bin_numeric"},
           "coef", "std_err", "z", "p_value"), csv_scale=1,
      wb=("Fig.3B-H", "Turn", "Stress x time", 0), note="per second"),
    C("Fig 3F locomotion x time", (0.005, 0.002, 2.553), scale=60,
      csv=("stats_figure3_overtime.csv",
           {"cluster": "Locomotion", "analysis": "Combined",
            "parameter": "group[T.ELS]:time_bin_numeric"},
           "coef", "std_err", "z", "p_value"), csv_scale=1,
      wb=("Fig.3B-H", "Locomotion", "Stress x time", 0), note="per second"),

    C("Fig 4E Simpson", (-0.013, 0.007, -1.893),
      csv=("stats_figure4_diversity_MixedLM.csv", {"metric": "Simpson Index"},
           "coef", "se", "z", "p_value"),
      wb=("Fig.4E-H", "Simpson", "Stress", 0)),
    C("Fig 4H CUI", (0.064, 0.030, 2.096),
      csv=("stats_figure4_diversity_MixedLM.csv", {"metric": "Cumulative usage index"},
           "coef", "se", "z", "p_value"),
      wb=("Fig.4E-H", "Cumulative usage", "Stress", 0)),
    C("Fig 4K freeze bout", None,
      csv=("stats_figure4_bouts_MixedLM.csv", {"cluster": "Freezing"},
           "coef", "se", "z", "p_value"),
      wb=("Fig.4J-Q", "Freezing", "Stress", 0), note="not quoted in the text"),
    C("Fig 4L sniff bout", (0.321, 0.147, 2.182),
      csv=("stats_figure4_bouts_MixedLM.csv", {"cluster": "Sniffing"},
           "coef", "se", "z", "p_value"),
      wb=("Fig.4J-Q", "Sniffing", "Stress", 0)),
    C("Fig 4N turn bout", (0.164, 0.071, 2.318),
      csv=("stats_figure4_bouts_MixedLM.csv", {"cluster": "Turn"},
           "coef", "se", "z", "p_value"),
      wb=("Fig.4J-Q", "Turn", "Stress", 0)),
    C("Fig 4U recurrence", (0.013, 0.007, 1.893),
      csv=("stats_figure4_transition_MixedLM.csv", {"metric": "Recurrence rate"},
           "coef", "se", "z", "p_value"),
      wb=("Fig.4T-W", "Recurrence", "Stress", 0)),
    C("Fig 4V determinism", (0.016, 0.009, 1.778),
      csv=("stats_figure4_transition_MixedLM.csv", {"metric": "Determinism"},
           "coef", "se", "z", "p_value"),
      wb=("Fig.4T-W", "Determinism", "Stress", 0)),
    C("Fig 4W Markov entropy", None,
      csv=("stats_figure4_transition_MixedLM.csv", {"metric": "Markov entropy"},
           "coef", "se", "z", "p_value"),
      wb=("Fig.4T-W", "Markov", "Stress", 0), note="not quoted in the text"),

    C("Fig 5B dynamics score", (0.336, 0.104, 3.222),
      csv=("stats_figure5_dynamics.csv",
           {"section": "Euclidean Distance - Combined full dataset", "parameter": "Stress"},
           "coef", "se", "z", "p_value"), csv_scale=1,
      wb=("Fig.5A-B", "Euclidean", "Stress", 0)),

    C("Fig 5C freeze res-vs-vuln", (0.409, 0.118, 3.475),
      csv=("fig5c_frequency_gee.csv", {"cluster": "Freeze", "contrast": "ELS/Resilient"},
           "beta", "SE", "z", "p"),
      wb=("Fig.5C", "Freeze", "ELS resilient - ELS vulnerab", 0)),
    C("Fig 5C turn res-vs-vuln", (-0.145, 0.033, -4.437),
      csv=("fig5c_frequency_gee.csv", {"cluster": "Turn", "contrast": "ELS/Resilient"},
           "beta", "SE", "z", "p"),
    ...
    C("Fig 5D freeze res-vs-vuln x time", (1.865, 0.249, 7.484), scale=2,
      csv=("stats_figure5_timecourse_MixedLM.csv",
           {"section": "Freeze - Combined full dataset", "parameter": "ELS resilient - ELS vulnerable x time (per minute)"},
           "coef", "se", "z", "p_value"), csv_scale=2,
      wb=("Fig.5D-J", "Freeze", "ELS resilient - ELS vulnerab", 1), note="per 30 s bin"),
    C("Fig 5D freeze res-vs-ctrl x time", (0.619, 0.238, 2.599), scale=2,
      csv=("stats_figure5_timecourse_MixedLM.csv",
           {"section": "Freeze - Combined full dataset", "parameter": "ELS resilient - Control x time (per minute)"},
           "coef", "se", "z", "p_value"), csv_scale=2,
      wb=("Fig.5D-J", "Freeze", "ELS resilient - Control x", 0), note="per 30 s bin"),
    C("Fig 5E sniff res-vs-vuln x time", (0.338, 0.176, 1.920), scale=2,
      csv=("stats_figure5_timecourse_MixedLM.csv",
           {"section": "Sniff - Combined full dataset", "parameter": "ELS resilient - ELS vulnerable x time (per minute)"},
           "coef", "se", "z", "p_value"), csv_scale=2,
      wb=("Fig.5D-J", "Sniff", "ELS resilient - ELS vulnerab", 1), note="per 30 s bin"),
    C("Fig 5G turn res-vs-vuln x time", (-1.622, 0.326, -4.967), scale=2,
      csv=("stats_figure5_timecourse_MixedLM.csv",
           {"section": "Turn - Combined full dataset", "parameter": "ELS resilient - ELS vulnerable x time (per minute)"},
           "coef", "se", "z", "p_value"), csv_scale=2,
      wb=("Fig.5D-J", "Turn", "ELS resilient - ELS vulnerab", 1), note="per 30 s bin"),

    C("Fig 6G Simpson vuln-vs-ctrl", (-0.022, 0.007, -3.017),
      csv=("fig6_diversity_resilience_stats.csv",
           {"metric": "Simpson", "contrast": "vuln_vs_control"}, "beta", "SE", "z", "p"),
      wb=("Fig.6G-K", "Simpson", "ELS vulnerable - Control", 0)),
    C("Fig 6G Simpson res-vs-vuln", (0.029, 0.010, 3.091),
      csv=("fig6_diversity_resilience_stats.csv",
           {"metric": "Simpson", "contrast": "resilient_vs_vuln"}, "beta", "SE", "z", "p"),
      wb=("Fig.6G-K", "Simpson", "ELS resilient - ELS vulnerab", 0)),
    C("Fig 6M freeze bout res-vs-vuln", (0.263, 0.100, 2.644),
      csv=("fig6_bout_resilience_stats.csv",
           {"cluster": "Freezing", "contrast": "resilient_vs_vuln"}, "beta", "SE", "z", "p"),
      wb=("Fig.6L-S", "Freeze", "ELS resilient - ELS vulnerab", 0)),
    C("Fig 6N sniff bout vuln-vs-ctrl", (0.420, 0.178, 2.364),
      csv=("fig6_bout_resilience_stats.csv",
           {"cluster": "Sniffing", "contrast": "vuln_vs_control"}, "beta", "SE", "z", "p"),
      wb=("Fig.6L-S", "Sniff", "ELS vulnerable - Control", 0)),
    C("Fig 6P turn bout res-vs-vuln", (-0.201, 0.103, -1.963),
      csv=("fig6_bout_resilience_stats.csv",
           {"cluster": "Turn", "contrast": "resilient_vs_vuln"}, "beta", "SE", "z", "p"),
      wb=("Fig.6L-S", "Turn", "ELS resilient - ELS vulnerab", 0)),
    C("Fig 6W Lempel-Ziv vuln-vs-ctrl", (-8.965, 5.224, -1.716),
      csv=("fig6_transition_resilience_stats.csv",
           {"metric": "LZ_complexity", "contrast": "vuln_vs_control"}, "beta", "SE", "z", "p"),
      wb=("Fig.6W-Z", "Lempel-Ziv", "ELS vulnerable - Control", 0)),
    C("Fig 6X recurrence vuln-vs-ctrl", (0.026, 0.008, 3.237),
      csv=("fig6_transition_resilience_stats.csv",
           {"metric": "Recurrence", "contrast": "vuln_vs_control"}, "beta", "SE", "z", "p"),
      wb=("Fig.6W-Z", "Recurrence", "ELS vulnerable - Control", 0)),
    C("Fig 6X recurrence res-vs-vuln", (-0.033, 0.010, -3.253),
      csv=("fig6_transition_resilience_stats.csv",
           {"metric": "Recurrence", "contrast": "resilient_vs_vuln"}, "beta", "SE", "z", "p"),
      wb=("Fig.6W-Z", "Recurrence", "ELS resilient - ELS vulnerab", 0)),
    C("Fig 6Y determinism vuln-vs-ctrl", (0.010, 0.004, 2.204),
      csv=("fig6_transition_resilience_stats.csv",
           {"metric": "Determinism", "contrast": "vuln_vs_control"}, "beta", "SE", "z", "p"),
      wb=("Fig.6W-Z", "Determinism", "ELS vulnerable - Control", 0)),
    C("Fig 6Z Markov vuln-vs-ctrl", (-0.044, 0.029, -1.502),
      csv=("fig6_transition_resilience_stats.csv",
           {"metric": "MarkovEntropyIdx", "contrast": "vuln_vs_control"}, "beta", "SE", "z", "p"),
...
# random intercept was dropped (see src.statistics.fit_grouped_or_ols). The
...
FIG4_STARS = [
    ...
    ("4F  Shannon entropy",        "stats_figure4_diversity_MixedLM.csv",  "metric",  "Shannon entropy",        False),
    ("4G  Evenness",               "stats_figure4_diversity_MixedLM.csv",  "metric",  "Evenness",               False),
    ("4H  Cumulative usage index", "stats_figure4_diversity_MixedLM.csv",  "metric",  "Cumulative usage index", True),
    ...
    ("4T  Lempel-Ziv",             "stats_figure4_transition_MixedLM.csv", "metric",  "Lempel-Ziv complexity",  False),
    ...
    ("4W  Markov entropy",         "stats_figure4_transition_MixedLM.csv", "metric",  "Markov entropy",         False),
]
...
FIG6_STARS = [
    ("6G  Simpson index",     "fig6_diversity_resilience_stats.csv",  "metric",  "Simpson",          [(0, 1), (1, 2)]),
    ...
    ("6X  Recurrence",        "fig6_transition_resilience_stats.csv", "metric",  "Recurrence",       [(0, 1), (1, 2)]),
    ("6Y  Determinism",       "fig6_transition_resilience_stats.csv", "metric",  "Determinism",      [(0, 1)]),
    ("6Z  Markov entropy",    "fig6_transition_resilience_stats.csv", "metric",  "MarkovEntropyIdx", []),
]
...
OVERALL_STARS = [
    ...
    ("6L  Overall bout [vuln_vs_control]",  ("Fig.6L-S", "Overall - Combined", "ELS vulnerable - Control", 0),       False),
    ("6L  Overall bout [resilient_vs_vuln]", ("Fig.6L-S", "Overall - Combined", "ELS resilient - ELS vulnerab", 0),  False),
]
...
def read_manuscript(path: Path) -> list[dict]:
    ...
    for m in STAT_RE.finditer(text):
        ...
        found.append(dict(
            beta=float(beta), se=float(se), z=float(z),
            p_op=op, p=float(p) if p is not None else None,
            bh_op=bh_op, bh=float(bh) if bh is not None else None,
...
def index_workbook(path: Path) -> list[dict]:
    ...
    for ws in wb.worksheets:
        ...
        heads = [(ri, ci) for ri, row in enumerate(grid) for ci, v in enumerate(row)
                 if isinstance(v, str) and v.strip() in ("Coef.", "coef")]
        for ri, ci in heads:
            section = ""
            for rj in range(ri - 1, -1, -1):
                for cj in range(ci, max(-1, ci - 12), -1):
                    if cj < len(grid[rj]) and isinstance(grid[rj][cj], str) and grid[rj][cj].strip():
            ...
            while rj < len(grid):
                ...
                out.append(dict(sheet=ws.title, section=section, label=str(label).strip(),
                                coef=vals[0], se=vals[1], z=vals[2], p=vals[3],
...
def csv_lookup(spec):
    ...
    if sub.empty:
        return None
    row = sub.iloc[0]
    return dict(coef=float(row[c_col]), se=float(row[s_col]),
                z=float(row[z_col]), p=float(row[p_col]))
...
def check_statistics(index, ms_stats, fails):
    ...
    for claim in CLAIMS:
        ...
        if unclaimed is not None and claim["ms"] is not None:
            for cand in unclaimed:
                if (close(cand["beta"], mb) and close(cand["se"], mse)
                        and close(cand["z"], mz)):
                    quoted = cand
        ...
        if claim["wb"] is None:
            ...
            if (quoted is not None and quoted["bh"] is not None
        ...
        if csv_row and wb_row:
            ok = (close(csv_row["coef"] / cscale, wb_row["coef"] / scale, CROSS_TOL)
    ...
    print(f"{'claim':<{width}}  {'manuscript':>10}  {'computation':>11}  {'report':>8}  {'comp=report':>11}  unit")
    print("-" * (width + 56))
    for panel, manuscript, computation, report, cross, note in rows:
        print(f"{panel:<{width}}  {manuscript:>10}  {computation:>11}  {report:>8}  {cross:>11}  {note}")
...
def check_figure_stars(index, fails):
    ...
    for panel, filename, key, row_key, starred in FIG4_STARS:
        ...
        checked += 1
        if (p < ALPHA) != starred:
            fails.append(f"{panel}: figure draws {'a star' if starred else 'no star'} but p={p:.4f}")
...
def main() -> int:
    ...
    print(f"Computation: statistics/*.csv")
    print(f"Figures    : significance markers declared in this script\n")
