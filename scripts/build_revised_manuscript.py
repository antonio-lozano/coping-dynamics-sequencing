...
SRC = (
    Path(sys.argv[1])
    if len(sys.argv) > 1
    else (Path.home() / "Downloads" / "Sanguino-Gomez_coping_strategies.docx")
)
...
# 3 - Supplementary Figure 5 is cited here, between the first mentions of
#     Supplementary Figures 3 and 5, so the supplementary figures are finally
#     numbered in order of appearance (they currently run 1,2,3,5,4). The
#     climbing-detection footage is folded into Supplementary Video 1 rather
#     than shipped as a separate Video 2.
...
if p is not None:
    replace(
        ...
        "geometrically from arena-floor overlap (Supplementary Fig. 5).",
        label="cite Supplementary Fig. 5",
    )
...
if p is not None:
    replace(
        p,
        "SE = 0.170, z = 1.970, p = 0.047, BH_FDR p = 0.109",
...
if p is not None:
    replace(
        p,
        "z = 2.972, p = 0.021; BH_FDR p = 0.021",
...
if p is not None:
    replace(
        p,
        "SE = 0.128, z = 3.285, p =  0.001, BH_FDR p = 0.004",
...
if p is not None:
    replace(
        p,
        "E. Boxplot of the Simpson diversity index (mean ± SEM). ELS reduces "
        "behavioral motif diversity.",
        "E. Boxplot of the Simpson diversity index (mean ± SEM). ELS shows a trend "
        "toward reduced behavioral motif diversity (p = 0.059).",
        label="Fig 4 legend E",
    )
    replace(
        p,
        "U. Boxplot of recurrence rate (mean ± SEM). ELS increases recurrence.",
        "U. Boxplot of recurrence rate (mean ± SEM). ELS shows a trend toward "
        "increased recurrence (p = 0.059).",
        label="Fig 4 legend U",
    )
    replace(
        p,
        "W. Boxplot of Markov entropy index (mean ± SEM). ELS reduces Markov entropy.",
        "W. Boxplot of Markov entropy index (mean ± SEM). Markov entropy does not "
        "differ significantly between groups (p = 0.106).",
        label="Fig 4 legend W",
    )
...
if p is not None:
    replace(
        p,
        "gave the similar result for both feature sets",
        "gave similar results for both feature sets",
        label="grammar: similar results",
    )
