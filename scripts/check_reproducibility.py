...
def check_supplementary_video(errors: list[str]) -> None:
    ...
    # The atlas shows the 25 cluster-mapped syllables (the seven named behavior
    # clusters in generate_supplementary_video_1.CLUSTERS), not the full 0-34
    # range: unmapped syllables have no behavioral label to illustrate.
    expected = {
        0,
        28,
        18,
...
def check_submission_manuscript(errors: list[str]) -> None:
    ...
    abstract_heading = next(
        (
            ...
            if paragraph.text == "Abstract (150 words)"
        ),
        None,
    )
    ...
    for addition in required_additions:
        ...
        if paragraph is None:
            ...
            continue
        highlighted = "".join(
            run.text for run in paragraph.runs if run.font.highlight_color == WD_COLOR_INDEX.YELLOW
        )
