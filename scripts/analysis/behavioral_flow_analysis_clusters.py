...
print(f"{len(meta)} animals ({is_c.sum()} Control / {is_e.sum()} ELS), "
...
for name, X in variants(counts).items():
    ...
    print(f"  {name:<32s} d={obs:8.3f}  percentile={pct:5.1f}  z={z:6.2f}  "
          f"p={p:.2e}  {star}")
...
for name, X in variants(counts).items():
    ...
    print(f"  {name}")
    print(f"     control-range rule : n={len(inside):2d}  "
          f"overlap with Euclid-12 = {len(inside & EUCLID12)/12*100:5.1f}%")
    print(f"     score < 0 rule     : n={len(below0):2d}  "
