...
def load_animal_metrics() -> pd.DataFrame:
    ...
    mean_bout = (
        bouts.groupby(["Animal", "cluster"], as_index=False)["bout_duration"]
        .mean()
        .pivot(index="Animal", columns="cluster", values="bout_duration")
    )
...
def structure_report(df: pd.DataFrame) -> pd.DataFrame:
    ...
    rows.append(
        {
            "quantity": "litters spanning both groups",
    ...
    rows.append(
        {
            "quantity": "litters containing a resilient animal",
    ...
    rows.append(
        {"quantity": "ELS litters split resilient/vulnerable", "value": int((mixed > 1).sum())}
    )
...
METRICS = {
    ...
    "markov": "Markov entropy",
    "Freezing": "Freeze bout duration",
    "Sniffing": "Sniff bout duration",
...
def refit_with_litter(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for column, label in METRICS.items():
        ...
        # coping_dynamics.statistics.fit_grouped_or_ols), so its SEs are not reproducible.
        ...
        for grouping, name in (("Animal", "animal (not identifiable)"), ("litter", "litter")):
            try:
                res = smf.mixedlm(
                    f"Q('{column}') ~ Condition + Experiment", d, groups=d[grouping]
                ).fit(reml=False)
                key = "Condition[T.ELS]"
                rows.append(
                    {
            ...
            rows.append(
                {
                    "metric": label,
                    "ICC_litter": icc,
                    "var_litter": var_litter,
                    "var_residual": var_resid,
                    "LR_stat": lr,
                    "p_litter_effect": p_lr,
                }
            )
        except Exception as exc:  # noqa: BLE001
            rows.append(
...
def sibling_similarity_test(df: pd.DataFrame) -> dict:
    ...
    meta = (
        freq[["animal_id", "group", "experiment"]]
    ...
    strata = pd.Series([f"{g}|{e}" for g, e in zip(meta["group"], meta["experiment"])])

    def mean_sibling(labels: np.ndarray) -> float:
        vals = [dist[i, j] for i in range(n) for j in range(i + 1, n) if labels[i] == labels[j]]
    ...
    return {
        ...
        "p_within_condition_and_cohort": float((np.sum(strat <= observed) + 1) / (len(strat) + 1)),
    }
...
def litter_size_effect(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for column, label in METRICS.items():
        ...
        try:
            res = smf.mixedlm(
                f"Q('{column}') ~ Condition + Experiment + litter_n", sub, groups=sub["litter"]
            ).fit(reml=False)
            rows.append(
                {
                    "metric": label,
                    "beta_litter_n": res.params["litter_n"],
                    "SE": res.bse["litter_n"],
                    "p": res.pvalues["litter_n"],
                }
            )
...
def main() -> None:
    ...
    counts = (
        df.groupby(["profile", "litter"])
        .size()
        .reset_index(name="n")
        .pivot(index="litter", columns="profile", values="n")
        .fillna(0)
        .astype(int)
    )
    ...
    print(
        f"  {test['resilient_n']} resilient animals come from "
        f"{test['observed_litters']} litters "
        f"(chance expectation {test['expected_litters']:.1f})"
    )
    print(
        f"  permutation p (more clustered than chance) = {test['p_more_clustered_than_chance']:.3f}"
    )
    ...
    print(
        f"  null, litter shuffled freely          = {sib['null_unstratified']:.2f}"
        f"   p = {sib['p_unstratified']:.4f}"
    )
    print(
        f"  null, shuffled within condition+cohort= "
    ...
    wide = fits.pivot(index="metric", columns="random_effect", values=["beta", "SE", "p"])
    ...
    print("\nCONCLUSIONS THAT CHANGE:", "none" if not flips else "")
