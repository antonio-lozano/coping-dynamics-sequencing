...
def loocv(Z, y):
    ...
    for i in range(len(y)):
        ...
        m[i] = False
        ok += int(
            LogisticRegression(max_iter=5000).fit(Z[m], y[m]).predict(Z[i : i + 1])[0] == y[i]
        )
    return ok / len(y) * 100
...
for kind in [
    ...
    for metric in ["euclidean", "cosine", "correlation", "cityblock", "jensenshannon"]:
        D = distance(X, metric)
        coords = MDS(
            n_components=2, dissimilarity="precomputed", random_state=42, normalized_stress=False
        ).fit_transform(D)
...
out["p_hypergeometric"] = [hypergeom.sf(h - 1, 41, 12, n) for h, n in zip(out.hits, out.n_res)]
out.to_csv(ROOT / "statistics" / "transition_representation_sweep.csv", index=False)
