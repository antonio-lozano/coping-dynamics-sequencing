...
Outputs:
    ...
    * docs/manuscript_statistics_validation/timecourse_reproduction.csv (Layer 2,
      only when the optional MoSeq cluster pickle is available)
"""
...
from src.config import (  # noqa: E402
    ...
    FPS,
    INDEX_CSV,
    MOSEQ_CLUSTER_PICKLE,
)

warnings.filterwarnings("ignore")  # silence MixedLM convergence chatter

OUT_DIR = _repo_root / "docs" / "manuscript_statistics_validation"
    ...
    for panel, label, beta, se, z, p_text, p_val in REPORTED:
        ...
        rows.append(
            dict(
                ...
                SE=se,
                z_reported=z,
                beta_over_SE=round(b_over_se, 3),
                p_text=p_text,
                p_from_z=round(p_recomputed, 4),
                flag=" ".join(flags),
            )
        )
...
def load_clusters():
    with open(MOSEQ_CLUSTER_PICKLE, "rb") as f:
            ...
            rows.append(
                {
                    ...
                    "pct": float(np.mean(block == code) * 100.0),
                }
            )
    return pd.DataFrame(rows)
...
def reproduce_timecourses(results, gmap) -> pd.DataFrame:
    ...
    for key, (code, term) in TIMECOURSE_PANELS.items():
        ...
        rows.append(
            dict(
                key=key,
                cluster=BEHAVIOR_MAPPING[code],
                term=term,
                beta_repro=round(b, 4),
                SE_repro=round(se, 4),
                z_repro=round(z, 3),
                p_repro=p,
                n_obs=fits[code]["n_obs"],
                n_animals=fits[code]["n_animals"],
            )
        )
    ...
    def verdict(r):
        ...
        if db <= 0.0025 and dz <= max(0.5, 0.12 * abs(r["z_reported"])):
            return "MATCH"
        if (r["beta_repro"] > 0) == (r["beta_reported"] > 0) and dz <= max(
            1.0, 0.25 * abs(r["z_reported"])
        ):
            return "CLOSE"
        return "CHECK"
...
def main():
    ...
    print("\n[LAYER 2] Data reproduction of the time-course mixed models")
    print("-" * 84)
    if not (MOSEQ_CLUSTER_PICKLE.exists() and INDEX_CSV.exists()):
        # The MoSeq cluster pickle is an optional development object, not part
        # of the repository. The tracked timecourse_reproduction.csv preserves
        # the Layer 2 record from the machine that had it.
        print(f"  SKIPPED - needs {MOSEQ_CLUSTER_PICKLE.name} and {INDEX_CSV.name},")
        print("  which are not bundled. The tracked timecourse_reproduction.csv is the record.")
        repro = None
    else:
    ...
    if repro is not None:
        ...
        print(f"  {OUT_DIR / 'timecourse_reproduction.csv'}")
