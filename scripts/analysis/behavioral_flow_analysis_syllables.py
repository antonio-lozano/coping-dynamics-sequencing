...
df = pd.read_csv(ROOT / "data/raw/moseq_syllables_per_frame.csv.gz",
                 usecols=["name", "syllable", "group", "frame_index"])
print(f"{len(df)} frames, {df.name.nunique()} animals, "
      f"{df.syllable.nunique()} distinct syllables")
...
print(f"retaining {len(kept)} syllables covering "
      f"{usage[keep].sum()*100:.1f}% of frames")
...
row = mats.sum(2, keepdims=True); row[row == 0] = 1
tot = mats.sum((1, 2), keepdims=True); tot[tot == 0] = 1
sets = {
...
for name, X in sets.items():
    ...
    print(f"  {name:<24s} d={obs:9.3f} percentile={pct:5.1f} z={z:6.2f} "
          f"p={p:.2e} {star}")
...
for name, X in sets.items():
    ...
    score = np.log((np.abs(X - med_c).sum(1) + 1e-9) /
                   (np.abs(X - med_e).sum(1) + 1e-9))
    lo, hi = score[is_c].min(), score[is_c].max()
    inside = set(meta.animal[(~is_c) & (score >= lo) & (score <= hi)])
    below = set(meta.animal[(~is_c) & (score < 0)])
    print(f"  {name}")
    print(f"     control-range: n={len(inside):2d} overlap={len(inside & EUCLID12)/12*100:5.1f}%")
