...
def loocv(Z):
    ...
    for i in range(len(y)):
        ...
        k[i] = False
        ok += int(
            LogisticRegression(max_iter=5000).fit(Z[k], y[k]).predict(Z[i : i + 1])[0] == y[i]
        )
    return ok / len(y) * 100
...
sets = {
    ...
    "dwell + collapsed off-diag": np.hstack(
        [np.array([np.diag(m) for m in P_full]), P_coll[:, off]]
    ),
}
        ...
        rows.append(
            {
                ...
                "LOOCV_mds2": loocv(co),
                "LOOCV_full": loocv(X),
                "welch_p": ttest_ind(score[is_c], score[is_e], equal_var=False).pvalue,
                "n_res": len(res),
