...
class FloorAnnotator:
    def __init__(self, root: tk.Tk, image_path: Path, initial: np.ndarray | None):
        ...
        self.save_button = tk.Button(
            bar,
            text="Save floor  (Enter)",
            command=self.save,
            font=("Segoe UI", 10, "bold"),
            bg="#2f7d32",
            fg="white",
            activebackground="#3a9c3e",
            relief="flat",
            padx=18,
            pady=7,
        )
    ...
    def on_click(self, event: tk.Event) -> None:
        ...
        if len(self.corners) >= 4:
            self.status.config(text="Four corners already placed - drag one, or press R to reset.")
            return
    ...
    def redraw(self) -> None:
        ...
        if len(self.corners) >= 2:
            ordered = (
                order_clockwise(np.array(self.corners, dtype=float))
                if len(self.corners) == 4
                else np.array(self.corners)
            )
        ...
        if placed < 4:
            self.status.config(text=f"{placed} of 4 corners placed - click {4 - placed} more.")
    ...
    def save(self) -> None:
        if len(self.corners) != 4:
            messagebox.showwarning("Arena floor", "Place all four corners before saving.")
            return
        corners = self.full_resolution()
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--image", type=Path, required=True, help="frame to annotate")
    parser.add_argument(
        "--corners", type=str, help="pre-load corners as 'x,y x,y x,y x,y' to adjust"
    )
...
def main() -> None:
    ...
    if initial is not None:
        ...
        temporary.corners = [[x * temporary.scale, y * temporary.scale] for x, y in initial]
        app = temporary
        app.redraw()
    else:
