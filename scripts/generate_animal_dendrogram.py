...
PROFILE_COLORS = {"Control": "#F9C74F", "ELS vulnerable": "#C37BA0",
                  "ELS resilient": "#5B9AAA"}
AXIS = "#4D4D4D"
...
def behavioural_profiles() -> tuple[pd.DataFrame, np.ndarray]:
    ...
    wide = timecourse.pivot_table(index="animal_id", columns=["cluster", "time_bin"],
    ...
    meta["profile"] = np.where(meta["group"] == "Control", "Control",
                               np.where(meta["animal"].isin(RESILIENT),
...
def main() -> None:
    ...
    dend = dendrogram(tree, labels=meta["animal"].to_numpy(), ax=ax,
                      color_threshold=None, above_threshold_color="#B0B0B0",
                      leaf_font_size=6.5)
    profile_of = dict(zip(meta["animal"], meta["profile"]))
    ax.set_ylabel("Ward linkage distance", fontsize=10, color=AXIS)
    ax.set_title("Animals clustered by 30 s behavioural profile "
                 "(condition not used in the clustering)", fontsize=11, color=AXIS, pad=10)
    ax.spines[["top", "right"]].set_visible(False)
    for side in ("left", "bottom"):
        ax.spines[side].set_color(AXIS)
    ax.tick_params(colors=AXIS, labelsize=8)
    ax.legend(handles=[plt.Line2D([], [], color=c, lw=6, label=k)
                       for k, c in PROFILE_COLORS.items()],
    ...
    share = share.pivot_table(index="animal_id", columns="cluster",
                              values="frequency_seconds", aggfunc="sum", fill_value=0.0)
    ...
    image = ax.imshow(share.to_numpy()[order], aspect="auto", cmap="magma_r",
    ...
    ax.set_title("Time share per behaviour, ordered by the dendrogram",
                 fontsize=11, color=AXIS, pad=10)
    ...
    meta[["animal", "group", "profile", "litter", "experiment", "tree_cluster"]] \
        .to_csv(OUT / "animal_cluster_assignments.csv", index=False)
    ...
    within = np.array([same_litter[i, j] for i in range(len(litter))
                       for j in range(i + 1, len(litter)) if litter[i] == litter[j]])
    between = np.array([same_litter[i, j] for i in range(len(litter))
                        for j in range(i + 1, len(litter)) if litter[i] != litter[j]])
