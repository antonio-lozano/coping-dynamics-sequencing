...
def render(fig6, pred, animal: str, profile: str, kind: str, out: Path) -> None:
    ...
    ax.set_title(
        f"Animal {animal} · {profile}",
        fontsize=6.2,
        color=fig6.AXIS,
        pad=14 if kind == "ethogram" else 10,
    )
    ...
    for extension in (".png", ".pdf"):
        figure.savefig(out / f"{animal}_{kind}{extension}", facecolor="white", bbox_inches="tight")
    plt.close(figure)
...
def main() -> None:
    ...
    parser.add_argument(
        "--animals", nargs="*", default=None, help="animal ids to render; default is all"
    )
    ...
    animals = args.animals or sorted(pred["Animal"].astype(str).unique(), key=lambda a: float(a))
    rows = []
