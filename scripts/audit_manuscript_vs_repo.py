...
def main() -> None:
    ...
    def check_panels(haystack: str, where: str) -> None:
        for m in re.finditer(r"(Supplementary\s+)?Fig(?:ure)?\.?\s*(\d+)\s*([A-Z](?:\s*[-\u2013]\s*[A-Z])?)",
                             haystack):
    ...
    n_refs = sum(1 for p in doc.paragraphs[ref_start:legend_start]
                 if p.style.name == "List Paragraph" and p.text.strip())
    if cited:
        ascending = cited == sorted(cited)
        add("4. References", "OK" if ascending else "ERROR",
        ...
        if not ascending:
            breaks = [(cited[i - 1], cited[i]) for i in range(1, len(cited))
                      if cited[i] < cited[i - 1]]
            add("4. References", "ERROR",
    ...
    for num in FIGURE_PANELS:
        ...
        add("5. Repo artifacts", "OK" if f.exists() else "ERROR",
    ...
    for num in SUPP_PANELS:
        ...
        add("5. Repo artifacts", "OK" if f.exists() else "ERROR",
            f"figures/supplementary_figure{num}.pdf "
            f"{'present' if f.exists() else 'MISSING'}.")

    # ---- write -------------------------------------------------------------
    lines = ["# Manuscript vs repository audit", "",
             f"Source document: `{DOC.name}`", ""]
