...
def title_frame() -> Image.Image:
    ...
    centered(
        draw,
        240,
        "Climbing is not recoverable from top-down keypoint MoSeq;",
        FONTS["body"],
        "#4A4A4A",
    )
    centered(
        draw,
        270,
        "it is labelled where the convex hull of the tracked points",
        FONTS["body"],
        "#4A4A4A",
    )
    centered(
        draw,
        300,
        "leaves the annotated arena floor (Supplementary Figure 5).",
        FONTS["body"],
        "#4A4A4A",
    )
    centered(
        draw, 370, "Representative occurrences of the resulting class", FONTS["body"], "#666666"
    )
...
def end_frame() -> Image.Image:
    ...
    centered(
        draw,
        235,
        "Frames with floor overlap below 0.8 are candidate climbing frames.",
        FONTS["body"],
        "#555555",
    )
    centered(
        draw,
        270,
        "Candidates are kept only in bouts longer than 425 ms (17 frames);",
        FONTS["body"],
        "#555555",
    )
    centered(
        draw,
        305,
        "shorter runs revert to the dominant non-climbing label.",
        FONTS["body"],
        "#555555",
    )
    return image
...
def render(frame: Image.Image, index: int, total: int) -> Image.Image:
    ...
    draw.text(
        (552, 140),
        f"Representative occurrence {index // OCCURRENCE_FRAMES + 1}"
        f" of {total // OCCURRENCE_FRAMES}",
        font=FONTS["small"],
        fill="#666666",
    )
