"""Test transition metrics models WITHOUT Experiment as covariate."""
import sys, warnings
...
def compute_markov_entropy_index(df, smoothing_factor=0.01):
    results = []
    df_sorted = df.sort_values(['Animal', 'Time_bin'])
    for animal, grp in df_sorted.groupby('Animal'):
...
print("Computing recurrence/determinism...")
rr_list, det_list = [], []
for animal, group in moseq_df.groupby('Animal'):
    R = recurrence_plot(group['Cluster'].tolist())
    rr_list.append({'Animal': animal, 'Condition': group['Condition'].iloc[0],
                    'Experiment': group['Experiment'].iloc[0], 'Recurrence_Rate': recurrence_rate(R)})
    det_list.append({'Animal': animal, 'Condition': group['Condition'].iloc[0],
                     'Experiment': group['Experiment'].iloc[0], 'Determinism': determinism(R)})
...
pk = "Condition[T.ELS]"
print(f"\nGold values from Report.xlsx:")
print(f"  Recurrence:  SE=0.00594, z=2.264, p=0.024")
print(f"  Determinism: SE=0.00476, z=3.382, p=0.001")
print(f"  Markov:      SE=0.01759, z=-2.342, p=0.019")

print(f"\n--- WITH Experiment in model ---")
for name, df, metric in [("Recurrence", rr_df, "Recurrence_Rate"),
    ...
    print(f"  {name}: beta={m.params[pk]:.5f} SE={m.bse[pk]:.5f} z={m.tvalues[pk]:.3f} p={m.pvalues[pk]:.5f}")

print(f"\n--- WITHOUT Experiment in model ---")
for name, df, metric in [("Recurrence", rr_df, "Recurrence_Rate"),
                          ("Determinism", det_df, "Determinism"),
                          ("Markov", entropy_merged, "MarkovEntropy")]:
    m = smf.mixedlm(f"{metric} ~ Condition", df, groups=df["Animal"]).fit(reml=False)
    print(f"  {name}: beta={m.params[pk]:.5f} SE={m.bse[pk]:.5f} z={m.tvalues[pk]:.3f} p={m.pvalues[pk]:.5f}")
