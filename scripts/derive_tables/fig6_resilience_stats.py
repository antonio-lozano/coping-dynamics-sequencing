...
ELS_VULN = ["15.6", "23.4", "25.1", "32.2", "32.4", "49.5", "55.3", "55.5", "56.3", "63.3",
            "69.1", "69.2", "69.3", "69.5", "73.1", "73.4", "75.1", "88.2", "88.3", "128.4",
            "133.3", "123.1", "134.4", "144.3", "144.5", "153.4", "154.4", "159.3", "159.5"]
...
def load():
    ...
    df["New_condition"] = df.apply(
        lambda r: "ELS_resilient" if r.Animal in ELS_RESILIENT
        else ("ELS" if r.Animal in ELS_VULN else r["Condition"]), axis=1)
    return df


def diversity_table(df):
    f = df.groupby(["Animal", "Experiment", "New_condition", "Cluster"]).size().reset_index(name="Count")
    piv = f.pivot_table(index=["Animal", "Experiment", "New_condition"], columns="Cluster",
    ...
    idx = H.index
    div = pd.DataFrame({"Animal": idx.get_level_values(0), "Experiment": idx.get_level_values(1),
                        "New_condition": idx.get_level_values(2),
                        "Entropy": H.values, "Evenness": E.values, "Simpson": S.values})
    # CUI (named clusters only)
    fc = df[df.Cluster != ""].groupby(["Animal", "Experiment", "New_condition", "Cluster"]).size().reset_index(name="Count")
    tota = fc.groupby(["Animal", "Experiment", "New_condition"])["Count"].sum().reset_index(name="Total")
    fc = fc.merge(tota, on=["Animal", "Experiment", "New_condition"])
    fc["rel"] = fc.Count / fc.Total
    def cui(s):
        rels = np.sort(s.values)[::-1]; cum = rels.cumsum(); n = len(cum)
        base = (n + 1) / (2 * n)
        return (cum.mean() - base) / (1 - base)
    cu = fc.groupby(["Animal", "Experiment", "New_condition"])["rel"].apply(cui).reset_index(name="CUI")
    ...
    for a, grp in df.groupby("Animal"):
        ...
        uni = list(pd.unique(sts)); M = len(uni)
        if M < 2:
            rows.append({"Animal": a, "MarkovEntropy": 0.0}); continue
    ...
    mk = pd.DataFrame(rows).merge(df[["Animal", "Experiment", "New_condition"]].drop_duplicates(), on="Animal")
    div = div.merge(mk[["Animal", "Experiment", "MarkovEntropy"]], on=["Animal", "Experiment"], how="left")
    return div


def bout_table(df):
    return df.groupby(["Animal", "Experiment"]).apply(
        lambda d: d.sort_values("Time_bin")
                   .assign(chg=lambda x: (x["Cluster"] != x["Cluster"].shift()).astype(int))
                   .assign(Bout=lambda x: x["chg"].cumsum())
                   .groupby(["Bout", "Cluster"]).size().mul(0.25)
                   .groupby(level=1).mean()
    ).reset_index(name="MeanBoutDuration").merge(
        df[["Animal", "Experiment", "New_condition"]].drop_duplicates(),
        on=["Animal", "Experiment"], how="left")
...
def determinism(seq, min_length=2):
    arr = np.asarray(seq); n = len(arr); total = 0; diag_sum = 0
...
def transition_table(df):
    ...
    for a, g in pred.groupby("Animal", sort=False):
        ...
        info = g.iloc[0]
        rows.append({"Animal": a, "Experiment": info["Experiment"], "New_condition": info["New_condition"],
                     "LZ_complexity": lz_complexity(seq), "Recurrence": recurrence_rate(seq),
                     "Determinism": determinism(seq), "MarkovEntropyIdx": markov_entropy_seq(seq)})
...
def contrasts(data, metric):
    ...
    return {
        ...
        "resilient_vs_control": (mc.params[kRc], mc.bse[kRc], mc.tvalues[kRc], float(mc.pvalues[kRc])),
...
def build(items, label_col):
    ...
    for name, c in items:
        for ctype, (b, se, z, p) in c.items():
            rows.append({label_col: name, "contrast": ctype, "beta": round(b, 4),
                         "SE": round(se, 4), "z": round(z, 4), "p": p})
    out = pd.DataFrame(rows)
    out["p_BH_FDR"] = np.nan
    for ctype in out.contrast.unique():
        m = out.contrast == ctype
        out.loc[m, "p_BH_FDR"] = multipletests(out.loc[m, "p"], method="fdr_bh")[1]
    out["p"] = out["p"].round(4); out["p_BH_FDR"] = out["p_BH_FDR"].round(4)
...
def main():
    ...
    div_items = [(v, contrasts(div, v)) for v in ["Entropy", "Evenness", "Simpson", "CUI", "MarkovEntropy"]]
    ...
    def show(title, dd, key):
        print(f"\n===== {title} =====")
        print(f"  {key:<13}{'contrast':<22}{'beta':>8}{'SE':>8}{'z':>8}{'p':>9}{'p_BH':>9}{'sig':>5}")
        for _, r in dd.iterrows():
            print(f"  {r[key]:<13}{r.contrast:<22}{r.beta:>8.3f}{r.SE:>8.3f}{r.z:>8.3f}"
                  f"{r.p:>9.4f}{r.p_BH_FDR:>9.4f}{('*' if r.sig_BH else ''):>5}")

    # transition metrics
    tr = transition_table(df)
    tr_items = [(v, contrasts(tr, v)) for v in ["LZ_complexity", "Recurrence", "Determinism", "MarkovEntropyIdx"]]
    ...
    print(f"  Freeze resilient-vs-vuln : {g('Freezing','resilient_vs_vuln')}   (MS 0.263/0.089/2.969/0.003, BH 0.023)")
    print(f"  Turn   resilient-vs-vuln : {g('Turn','resilient_vs_vuln')}   (MS -0.201/0.089/-2.264/0.024, BH 0.085)")
    print(f"  Sniff  vuln-vs-control   : {g('Sniffing','vuln_vs_control')}   (MS 0.420/0.127/3.318/<.001, BH 0.004)")
