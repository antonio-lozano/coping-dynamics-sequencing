...
VIDEO_TITLE = (
    "Supplementary Video 1. Representative MoSeq syllables and canonical skeleton trajectories."
)
VIDEO_LEGEND = (
...
def changed_intervals(old_text: str, new_text: str) -> list[tuple[int, int]]:
    ...
    old_tokens = [
        (match.group(), match.start(), match.end()) for match in TOKEN_RE.finditer(old_text)
    ]
    new_tokens = [
        (match.group(), match.start(), match.end()) for match in TOKEN_RE.finditer(new_text)
    ]
...
def write_si_guide() -> None:
    ...
    guide.add_paragraph("Coping dynamics reveal individual resilience following early-life stress")
