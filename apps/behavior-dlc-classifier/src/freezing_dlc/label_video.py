...
def annotate_video_in_subprocess(
    video_path: Path,
    out_csv: Path,
    fps: float,
    existing_labels: Optional[Path] = None,
) -> dict:
    """Run the native OpenCV window on a main thread, separate from Tk.

    A fresh result file distinguishes cancel from an earlier saved labels file.
    The GUI worker waits here while Tk remains responsive in the parent.
    """
    with tempfile.TemporaryDirectory(prefix="freezing-label-") as temp:
        result_path = Path(temp) / "result.json"
        command = [
            sys.executable, "-m", "freezing_dlc.cli", "label-video",
            "--video", str(video_path), "--out-csv", str(out_csv),
            "--fps", str(fps), "--start-paused", "--result-json", str(result_path),
        ]
        if existing_labels is not None:
            command.extend(["--existing-labels", str(existing_labels)])
        completed = subprocess.run(command, capture_output=True, text=True)
        if completed.returncode:
            detail = completed.stderr.strip() or completed.stdout.strip()
            raise RuntimeError(f"Video labeler exited with code {completed.returncode}: {detail[-4000:]}")
        if not result_path.exists():
            raise RuntimeError("Video labeler exited without completion metadata; labels are unconfirmed.")
...
def annotate_video_labels(
    ...
    try:
        while True:
            ...
            # The controls extend to y=510. Small source frames previously
            # cropped the save/undo instructions and summary out of the canvas.
            # Resize only the display copy, preserving aspect ratio and all
            # source pixels/annotation frame indices used by the workflow.
            source_height, source_width = frame.shape[:2]
            height = max(520, min(720, source_height))
            width = max(1, round(source_width * height / source_height))
            display_frame = cv2.resize(frame, (width, height)) if height != source_height else frame
            ...
            # While paused, retain a position selected by the mouse callback
            # during waitKeyEx; restoring frame_idx here discards every scrub.
    finally:
        cap.release()
