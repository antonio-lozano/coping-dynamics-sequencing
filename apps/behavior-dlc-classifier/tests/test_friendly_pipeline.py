...
@pytest.mark.parametrize("overrides", [
    {"bin_sec": 30.5}, {"fps": float("nan")},
    {"threshold": float("nan")}, {"threshold": 1.1},
    {"box_side_cm": float("inf")}, {"ethogram_min_bout_sec": -1},
])
def test_invalid_settings_fail_before_creating_results(tmp_path, overrides):
    model = tmp_path / "model.sav"
    model.touch()
    settings = _settings(tmp_path, model_path=model, run_dlc=False,
                         run_behavior_analysis=False, **overrides)
    with pytest.raises(ValueError):
        run_friendly_pipeline(settings, log=lambda _: None)
    assert sorted(p.name for p in tmp_path.iterdir()) == ["model.sav"]


def test_duplicate_recording_identities_rejected_before_copy(tmp_path):
    from freezing_dlc.friendly_pipeline import _prepare_original_videos

    project = tmp_path / "input"
    project.mkdir()
    for name in ("sessionA_Trial9_mouse3.mp4", "sessionB_Trial9_mouse3.mp4"):
        (project / name).touch()
    output = tmp_path / "output"
    output.mkdir()
    with pytest.raises(ValueError, match="same recording identity") as error:
        _prepare_original_videos(project, output, ".mp4", lambda _: None)
    assert "sessionA_Trial9_mouse3.mp4" in str(error.value)
    assert "sessionB_Trial9_mouse3.mp4" in str(error.value)
